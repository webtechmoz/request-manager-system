...
from Auxiliar.GenRequest import GENREQUEST
...
def adminPanel(page: ft.Page, username: str = 'Admin'):
    ...
    def archive_requests(e: ft.ControlEvent):
        ...
        def manage_request(e: ft.ControlEvent, id: int):
            
            def delete_request(e: ft.ControlEvent):
                ...
                editar_dados(
                    nomeTabela='pendentes',
                    dados=['status', 'Pendente', f'id = "{id}"']
                )
                
                snack_bar = SnackBar(value=f'Requisição nº {id} apagada com sucesso',icon=ft.icons.DELETE, color=ft.colors.RED)
                page.overlay.append(snack_bar)
                snack_bar.open = True
            ...
            def print_request(e: ft.ControlEvent):
                ...
                genrequest = GENREQUEST(dados)
                response = genrequest.manage_request()
                
                snack_bar = SnackBar(value=f'{response}',icon=ft.icons.WARNING, color=ft.colors.BLUE)
                page.overlay.append(snack_bar)
                snack_bar.open = True
            ...
            if e.control.icon == 'delete':
                alertdialog = AlertDialog(page, f'Requisição nº {id}', f'Deseja apagar a requisição nº {id}?','Apagar', ft.icons.DELETE, ft.colors.RED, delete_request)
                page.overlay.append(alertdialog)
                alertdialog.open = True
            
            elif e.control.icon == 'print':
                alertdialog = AlertDialog(page, f'Requisição nº {id}', f'Deseja imprimir a requisição nº {id}?','Imprimir', ft.icons.PRINT, ft.colors.BLUE, print_request)
                page.overlay.append(alertdialog)
                alertdialog.open = True
            
            # snack_bar = SnackBar(value=f'{e.control.icon}, id: {id}',icon=ft.icons.MENU_BOOK, color=ft.colors.BLUE)
            # page.overlay.append(snack_bar)
            # snack_bar.open = True
                    ...
                    ft.DataRow(
                        cells=[
                            ft.DataCell(
                                ft.Row(
                                    controls=[
                                        IconButton(icon=ft.icons.PRINT,color=ft.colors.BLUE, size=18, on_click=lambda e, id = int(dado[0]): manage_request(e, id)),
                                        IconButton(icon=ft.icons.DELETE,color=ft.colors.RED, size=18,  on_click=lambda e, id = int(dado[0]): manage_request(e, id))
                                    ]
                                )
                            ),
